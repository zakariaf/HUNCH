...
At PROBE's longest par, `N = 29`, that is a fixed 9 pt pitch and a 261 pt row on SE — always inside the 288 pt the bar allows, so within PROBE the row's length is exactly proportional to par and sec-10 §10.5's signal is intact. The clamp engages only in DRIFT, at **bands 7 and 8** (sec-07 §7.7): `par_DRIFT` is 37 and 40, and 37 × 9 = 333 and 40 × 9 = 360 both exceed 288, so the pitch compresses to 288/37 = **7.8 pt** and 288/40 = **7.2 pt** on SE, and to 9.4 and 8.7 pt on Pro Max. Bands 5–6 land exactly on the boundary — `32 × 9 = 288`, the whole row budget, with zero slack — which is the row to check first if either number is ever retuned. The tick itself stays 2 pt wide, leaving ≥ 5.2 pt of gap, and DRIFT's tick count already identifies the mode by design (sec-07 §7.5), so compressing its row costs no signal that was not already given away.
